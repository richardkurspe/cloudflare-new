# **⚠️ 来自Richard的~~踩坑~~补充说明

## 必要的环境变量

| 变量                              | 说明                                         | 可选值                           | 默认值                                                                                                                     |
| --------------------------------- | -------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| u                          | 访问终端时的密码&协议UUID           | UUID                       | （空）                                                                                                                     |
| C                          | 链接KV数据库时添加变量           | C                       | （空）                                                                                                                     |

1️⃣:项目部署完成后切勿将"自定义路径(d)"设置为"/",此举会导致与KV数据库的链接中段，后续只能通过删除重建KV数据库并重新连接部署到项目的方式解决。

2️⃣:如果项目部署后,所有协议测试结果均为-1ms的话。请检查pages设置页面的"兼容日期"选项，更正为"2026-01-20"后问题可解决。
